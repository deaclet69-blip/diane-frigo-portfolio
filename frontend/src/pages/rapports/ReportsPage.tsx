...
function TraceabilitySection() {
  ...
  function exportCsv() {
    ...
    const lines = data.rows.map((r) => [
      r.label,
      Math.round(r.recettes),
      Math.round(r.chiffreAffaires),
      Math.round(r.coutMarchandises),
      Math.round(r.margeBrute),
      Math.round(r.charges),
      Math.round(r.resultatNet),
    ]);
    lines.push([
      'TOTAL',
      Math.round(data.totals.recettes),
      Math.round(data.totals.chiffreAffaires),
      Math.round(data.totals.coutMarchandises),
      Math.round(data.totals.margeBrute),
      Math.round(data.totals.charges),
      Math.round(data.totals.resultatNet),
    ]);
  ...
  const searchLabel: Record<TraceabilityGranularity, string> = {
    day: 'Search a specific day',
    week: 'Search a week (pick any day in that week)',
    month: 'Search a specific month',
          ...
          <TableContainer sx={isMobile ? { boxShadow: 'inset -8px 0 6px -6px rgba(0,0,0,0.15)' } : undefined}>
            <Table stickyHeader size="small">
              <TableHead>
                <TableRow>
                  <TableCell
                    sx={isMobile ? { position: 'sticky', left: 0, bgcolor: 'background.paper', zIndex: 3 } : undefined}
                  >
                    Period
                  </TableCell>
              ...
              <TableBody>
                {displayRows.map((r) => (
                  <TableRow key={r.period} hover>
                    ...
                    <TableCell align="right" sx={{ color: diane.red }}>
                      {formatFcfa(r.charges)}
                    </TableCell>
                    <TableCell
                      align="right"
                      sx={{ fontWeight: 700, color: r.resultatNet >= 0 ? diane.green : diane.red }}
                    >
              ...
              {data && data.rows.length > 0 && (
                <TableBody>
                  <TableRow
                    sx={{ bgcolor: diane.bg, '& td': { fontWeight: 800, borderTop: `2px solid ${diane.navy}` } }}
                  >
                    <TableCell
                      sx={isMobile ? { position: 'sticky', left: 0, bgcolor: diane.bg, zIndex: 1 } : undefined}
                    >
                      TOTAL
                    </TableCell>
                    ...
                    <TableCell align="right" sx={{ color: diane.red }}>
                      {formatFcfa(data.totals.charges)}
                    </TableCell>
      ...
      {section === 'entrees' && (
        <Paper>
          <Box sx={{ p: 2.5, pb: 0 }}>
            ...
            <Typography variant="caption" color="text.secondary">
              Date, supplier, and purchase price for each restocking
            </Typography>
          </Box>
          {isMobile && (
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
                  ...
                  <TableRow key={v.productId}>
                    ...
                    <TableCell
                      ...
                      sx={{
                        fontWeight: 700,
                        color:
                          v.daysOfStockRemaining !== null && v.daysOfStockRemaining < 7
                            ? diane.red
                            : v.daysOfStockRemaining !== null && v.daysOfStockRemaining < 15
                              ? diane.orange
                              : diane.green,
                      }}
                    >
