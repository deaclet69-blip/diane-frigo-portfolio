...
export default function LoansPage() {
  ...
  if (notFound) {
    return (
      <Box maxWidth={480}>
        <Typography variant="h5" fontWeight={700} sx={{ mb: 2 }}>Investment</Typography>
        <Alert severity="info" sx={{ mb: 2 }}>
          No loan recorded yet.
        </Alert>
        <Button variant="contained" onClick={() => setOpen(true)}>Set Up My Loan</Button>
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
        <Typography variant="h5" fontWeight={700}>Investment</Typography>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        {status.repayment.isFullyRepaid ? (
          <Chip label="Loan fully covered by revenue" size="small" sx={{ bgcolor: '#E7F7EE', color: diane.green, fontWeight: 700 }} />
        ) : (
          <Typography variant="caption" color="text.secondary">
            Remaining to collect: {formatFcfa(status.repayment.remainingToSell)}
            {status.projections.daysToRepayLoan !== null && ` — estimated ${formatDays(status.projections.daysToRepayLoan)} at the current pace`}
          </Typography>
        )}
      </Paper>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Typography variant="caption" color="text.secondary">
          ...
          {status.projections.daysToProfitObjective !== null && ` — estimated ${formatDays(status.projections.daysToProfitObjective)} at the current pace`}
        </Typography>
      </Paper>

      <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ mb: 3 }}>
        <Paper sx={{ p: 2.5, flex: 1 }}>
          <Typography variant="caption" color="text.secondary">Recent Pace (last 14 days)</Typography>
          <Typography variant="h6" fontWeight={700}>{formatFcfa(status.projections.avgDailyProfit)}/day profit</Typography>
        </Paper>
        <Paper sx={{ p: 2.5, flex: 1 }}>
          <Typography variant="caption" color="text.secondary">Days of Stock Remaining</Typography>
          <Typography variant="h6" fontWeight={700}>
            {status.projections.stockRunwayDays !== null ? `~${Math.ceil(status.projections.stockRunwayDays)} days` : '—'}
          </Typography>
        </Paper>
        {status.projections.requiredDailyProfitBeforeStockOut !== null && (
          <Paper sx={{ p: 2.5, flex: 1 }}>
            <Typography variant="caption" color="text.secondary">Required Profit/Day Before Stockout</Typography>
      ...
      {status.profitObjective.remainingToGenerateProfit > 0 && (
        <Paper>
          <Box sx={{ p: 2.5, pb: 0 }}>
            <Typography variant="subtitle1" fontWeight={700}>By Product — to reach the target</Typography>
          </Box>
          <TableContainer>
<Table sx={{ mt: 1 }}>
            <TableHead>
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="xs">
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          <TextField
            label="Total Loan Amount (USD)" type="number" value={form.totalAmount}
            onChange={(e: any) => setForm({ ...form, totalAmount: e.target.value })} fullWidth
          />
          <TextField
            label="Of which construction (USD)" type="number" value={form.constructionAmount}
            onChange={(e: any) => setForm({ ...form, constructionAmount: e.target.value })} fullWidth
          />
          <TextField
            label="Of which equipment (USD)" type="number" value={form.equipmentAmount}
            onChange={(e: any) => setForm({ ...form, equipmentAmount: e.target.value })} fullWidth
          />
          <TextField
            label="Of which other (USD)" type="number" value={form.otherAmount}
            onChange={(e: any) => setForm({ ...form, otherAmount: e.target.value })} fullWidth
          />
          <TextField
            label="Custom Profit Target (optional)" type="number" value={form.customProfitGoal}
